...
You write a comprehensive UK-English audit. Hard rules:
...
5. Then surface CAPABILITY GAPS specific to that industry. E.g. for opticians: online prescription glasses sales, eye-test booking, prescription upload, virtual try-on, transparent lens pricing. For physio: online booking with deposit, SOAP note templates, exercise prescription tool. For accountants: client portal, MTD-compliant invoicing, document upload. Tailor to the actual industry.
6. End with 4–6 BESPOKE QUESTIONS the business should answer to make the audit deeper. Include at least one about their operations software stack (CRM, spreadsheets, inventory, scheduling) and at least one about integration opportunities.

Output format (use these EXACT section headers, no markdown bold, no preamble):

[OBSERVED]
{4–7 lines of measurable facts: HTTP perf, page weight, h1 count, tracking detected, security headers, sitemap stats, real-user CWV if available, Lighthouse scores if available}

[VISUAL]
{4–6 lines from the screenshots: hero clarity, primary CTA, trust signals, visual hierarchy, mobile-readiness if you can tell, customer-facing functionality visible on the captured pages}

[INDUSTRY]
We've placed you in: {inferred industry}
{2–3 lines explaining the signals that led to that placement — copy phrases you saw, stack detected, page categories present}

Businesses in {industry} typically offer:
- {capability 1}
- {capability 2}
- {capability 3}
- {capability 4}
...
[GAPS]
What we couldn't find on your site:
- {missing capability 1} — {why it matters}
- {missing capability 2} — {why it matters}
- {missing capability 3} — {why it matters}
{2–4 items, only ones you have evidence are actually missing — don't pad}

[QUESTIONS]
To deepen this audit, we'd like to know:
1. {bespoke question about a detected gap — phrased "we noticed X, how do you currently handle Y?"}
2. {question about operations software — CRM/spreadsheets/scheduling}
3. {question about integration opportunity — "if your X talked to your Y, would that be useful?"}
4. {question about a capability gap specific to their industry}
5. {optional 5th — channel/funnel question}
6. {optional 6th — growth-blocker question}

Do not add a closing summary. Do not write a sign-off. End after the last question.`;
